...
// 一个 range 在三节点上的三副本的一个 raft group 之间有网路通信。但是如果 range 变多的话，就会很复杂，
// 是四个节点之间大概8个 (如果我没看错颜色的话) range 的 raft group 之间通信。
// 很明显网络 RoundTrip 太多了。针对这个的优化就是 MultiRaft 了，说白了就是把网络请求 batch up，
// 这样网络压力就会小很多。针对这个的优化和应用本身的实现是耦合的，作者也尝试把这个优化加到 etcd/raft 当中，
// 但是最后的决定是增加 RawNode 这样的无线程安全的接口给应用层实用，方便对心跳等请求的合并，所以你会看到
// 有个非线程安全的RawNode的实现。具体到 cockroach 的实现是可以找到相应的代码的，省略无关代码就如下。
...
// 用给定的配置和raft peer列表，返回一个新的RawNode
...
// 当RawNode用户需要检查是否有Ready待处理时，调用HasReady。
// 此方法中的检查逻辑应与Ready.containsUpdates（）一致。
...
// Advance会通知RawNode该应用程序已应用并在最近的Ready结果中保存了进度。
